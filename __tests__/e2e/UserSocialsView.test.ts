...
test('renders table first row', async ({ page }) => {
  ...
  await expect(
    await page
      .locator('data-testid=UserSocialsView >> tbody tr:visible')
      .first()
      .locator('td')
      .allTextContents(),
  ).toEqual(expected)
...
test('renders pagination', async ({ page }) => {
  ...
  await expect(
    await page.locator('data-testid=UserSocialsView >> .ant-pagination'),
  ).toBeVisible()
})
...
test('pagination next page click works correctly', async ({ page }) => {
  ...
  const paginationLocator = await page.locator(
    'data-testid=UserSocialsView >> .ant-pagination',
  )
  ...
  await expect(
    await socialsViewLocator
      .locator('tbody tr:visible')
      .first()
      .locator('td')
      .allTextContents(),
  ).toEqual(expectedSecondPageFirstRow)
})
...
test('filters users on type input change', async ({ page }) => {
  ...
  const socialsViewLocator = await page.locator('data-testid=UserSocialsView')
  await page
    .locator('data-testid=UserSocialsView >> data-testid=SearchInput')
    .type('vi')

  await page.waitForTimeout(100)

  await expect(
    await socialsViewLocator.locator('tbody tr:visible').elementHandles(),
  ).toHaveLength(3)
  await expect(
    await socialsViewLocator
      .locator('tbody tr:visible')
